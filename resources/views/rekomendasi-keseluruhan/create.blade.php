...
@section('content')
    <div class="max-w-2xl mx-auto px-6 py-10 bg-white border border-gray-200 rounded-3xl shadow-xl">
        <h2 class="text-3xl font-bold text-[#7083C3] mb-8 tracking-tight">📝 Tambah Rekomendasi PBD</h2>
...
=======
            @foreach (['kategori' => 'Identifikasi Prioritas', 'masalah' => 'Akar Masalah'] as $field => $label)
>>>>>>> 7ea609cca1b6aba9b84198f84b3d17cec9ad1cd8
            ...
            @foreach (['kegiatan_benahi' => 'Kegiatan Pembenahan', 'kegiatan_arkas' => 'Kegiatan ARKAS'] as $name => $label)
                <div class="form-group">
                    <textarea name="{{ $name }}" id="{{ $name }}" rows="4" required placeholder=" "
                        class="form-input w-full px-4 pt-6 pb-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7083C3] focus:border-[#7083C3] transition @error($name) border-red-500 @enderror">{{ old($name) }}</textarea>
