...
<div class="max-w-7xl mx-auto px-4 py-10">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('rekomendasi-prioritas.create') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
            + Tambah Rekomendasi
        </a>
    </div>
                ...
                @forelse ($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border px-4 py-2 space-x-2">
                            <a href="{{ route('rekomendasi-prioritas.edit', $item->id) }}" class="text-blue-600 hover:underline">Edit</a>
                            <form action="{{ route('rekomendasi-prioritas.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus?')">
                                @csrf @method('DELETE')
